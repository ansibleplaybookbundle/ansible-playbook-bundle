...
def cmdrun_init(**kwargs):
    ...
    apb_name_arr = kwargs['name'].split('/')
    apb_name = apb_name_arr[-1]

    organization = kwargs['org']
    if organization is None:
        if len(apb_name_arr) > 1:
            organization = apb_name_arr[-2]
        else:
            raise Exception('Organization must be specified "apb init org/apb-name" or with the --org flag')
